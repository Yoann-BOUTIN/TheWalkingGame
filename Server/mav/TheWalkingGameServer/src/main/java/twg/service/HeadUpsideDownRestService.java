...
@Component
@Path("/headupsidedown")
public class HeadUpsideDownRestService {

	@Autowired
	private HeadUpsideDownDao headUpsideDownDao;
	...
	public Response findById(@QueryParam("id") String id) throws JsonGenerationException, JsonMappingException, IOException {
		...
		HeadUpsideDown dataById = headUpsideDownDao.getDataById(idLong);
	...
	@Transactional
	public Response updateHeadUpsideDownById(HeadUpsideDown headUpsideDown) {
		String message;
		int status;
		if (headUpsideDown.getId() != null) {
			headUpsideDownDao.updateHeadUpsideDown(headUpsideDown);
			status = 200; // OK
			message = "Game HeadUpsideDown has been updated";
		} else {
